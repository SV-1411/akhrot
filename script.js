(function () {
    ...
    if (!rafPending) {
      ...
      requestAnimationFrame(() => {
        ...
        if (window._videoDuration && window._video) {
          ...
          window._video.currentTime = Math.max(0, Math.min(window._videoDuration, t));
        }
      });
    }
  }
  ...
  /* ═════════════════════════════════════════════════════
     VIDEO INIT — multi-fallback for autoplay restrictions
  ═════════════════════════════════════════════════════ */
  ...
  if (video) {
    ...
    setTimeout(() => {
      ...
      const p = video.play();
      if (p && p.then) {
        p.then(() => { video.pause(); video.currentTime = 0; setVideoReady(); })
         .catch(() => {
           const tryOnInteraction = () => {
             video.play().then(() => { video.pause(); video.currentTime = 0; setVideoReady(); }).catch(() => {});
           };
           document.addEventListener('scroll',     tryOnInteraction, { once: true, passive: true });
           document.addEventListener('click',      tryOnInteraction, { once: true });
           document.addEventListener('touchstart', tryOnInteraction, { once: true, passive: true });
         });
      }
    }, 200);
    ...
    const pollInterval = setInterval(() => {
      pollCount++;
      setVideoReady();
      if (videoReady || pollCount > 30) clearInterval(pollInterval);
    }, 500);
  }
